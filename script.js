...
function configurarEventListenersApp() {
    ...
    if (clearDataPasswordInput) {
        clearDataPasswordInput.addEventListener('keydown', (event) => {
            if (event.key === 'Enter') {
                event.preventDefault(); // Impede o envio padrão do formulário e o reload
                executarLimpezaTotal(); // Dispara a função de limpeza diretamente
            }
        });
    }
            ...
            // Cenário A: Preencheu Nome -> Busca pelo Nome
                ...
                try {
                    ...
                    if (matches.length === 1) {
                        ...
                        if(matches.length === 0) console.log("Nenhum paciente encontrado com este nome exato.");
                        else console.warn("Múltiplos pacientes encontrados. Refine a busca.");
                    }
                } catch (e) {
...
function executarLimpezaTotal() {
    ...
    if (passwordInput.value === 'apocalipse') {
        // [ARCOSAFE-FIX] Limpeza Remota (Firebase) + Local
        if (database) {
