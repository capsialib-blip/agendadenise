...
function configurarEventListenersApp() {
    ...
    if (btnGerarRelatorioMensal) {
        ...
        if (reportMonthInput) reportMonthInput.value = `${year}-${month}`;
    }
}
...
function configurarAutopreenchimento(form) {
    ...
    inputs.forEach(input => {
        ...
        // 1. Lógica de Busca Cruzada ao sair do campo (Blur/Change)
        input.addEventListener('change', () => {
            ...
            if (match) {
                preencherCamposFormulario(form, match);
            }
        });
        ...
        sugestoesLista.addEventListener('mousedown', (e) => {
            ...
            if (item) {
                ...
                if (paciente) {
                    preencherCamposFormulario(form, paciente);
                }
                sugestoesLista.innerHTML = '';
                sugestoesLista.style.display = 'none';
            }
...
function preencherCamposFormulario(form, dados) {
    const campos = {
        'numero': dados.numero,
        'nome': dados.nome,
        'cns': dados.cns,
...
function verificarDuplicidadeAoDigitar(input, data, turno, vaga) {
    // Mantido vazio para compatibilidade futura
}
